	        ...
	        $("#handReversal").attr('value','正在交易，请稍后...');
	          $.ajax({
	               url:'${ctx}/purse/bagHandReversal',
	                     ...
	                     error:function()
	                     {
	                    	 W.$.dialog({title:'提示',content: '请求失败，请稍后再试!',max: false,min: false,parent:api,cancelVal: '关闭',cancel: function() {window.location.reload();}});
	                     ...
	                     {       
	                    	 $("#handReversal").attr('disabled',false);
	                    	 W.$.dialog({title:'提示',content: msg,max: false,min: false,parent:api,cancelVal: '关闭',cancel: function() {window.location.reload();}});
	                     }
	               });
...
<div class="item liHeight">
	...
	<ul>
		 ...
		 <li style="width:310px;" id="statusDesc"><span style="width:100px;">审核时间：</span>
			...
			<c:if test="${is_back==1}">
				<c:if test="${back_status==0||back_status==2||back_status==3||back_status==null}">
