...
<head>
	...
	<script type="text/javascript">
		...
		var INTEGER_REG_fee=  /^(?:0|[1-9][0-9]?|100)$/;///^(0|([1-9][0-9]{0,1}))$/; //非负整数且是两位：0~99.
			...
			if(!fr_flag){
			   ...
			   var val=$.trim($(n).val());
			    ...
			    if(!INTEGER_REG_fee.test(val)){
			      fr_flag=false;	
						     	var dialog = $.dialog({title: '验证分润规则',lock:true,content: '分润规则:费率必须在0%~99%之间！',icon: 'error.gif',ok: function(){
						         	$(n).focus().select();
								    }
								});
			     return false;
   ...
   if(locked_status!=1){
      ...
      //超级刷代理商行业计算成本-------end
				...
				if(agentArea ==null || agentArea==''){
					var dialog = $.dialog({title: '错误',lock:true,content: '超级刷代理商区域不能为空',icon: 'error.gif',ok: function(){
			        	$("#agentArea").focus();
					    }
					});
					return false;
				...
				if(agentAreaType == '-1')
				{
					var dialog = $.dialog({title: '错误',lock:true,content: '请选择超级刷代理商级别',icon: 'error.gif',ok: function(){
				        	$("#agentAreaType").focus();
					    }
					});
					return false;
				...
				if(agentPay.length != 0){
					...
					{
						var dialog = $.dialog({title: '错误',lock:true,content: '超级刷代理商接入费应为数字， 正确格式例子是：8.88或8',icon: 'error.gif',ok: function(){
				        	$("#agentPay").focus();
						    }
						});
		...
		function successMsg(){
			var dialog = $.dialog({title: '提示',lock:true,content: '超级刷代理商修改成功',icon: 'success.gif',ok:null });
		}
		...
		function checkAgentName(agentName,agentNo){
			var mark = true;
			 $.ajax({
		 			type:"post",
		 			url:"${ctx}/agent/agentNameCheckSPOS",
...
<body>
  <div id="content">
    <div id="nav"><img class="left" src="${ctx}/images/home.gif"/>当前位置：超代管理>超代修改</div>
    ...
    <div class="item">
    	...
    	<ul>
			<li><label>超代名称：</label><input type="text" id="agentName"  name="agentName"  value="${params.agent_name}"/><label class="must">*</label></li>
			<li><label>超代联系人：</label><input type="text" id="agentLinkName"  name="agentLinkName"  value="${params.agent_link_name}"/><label class="must">*</label></li>
			<div class="clear"></div>
			...
			<div class="clear"></div>
			<li><label>超代区域：</label><input type="text" id="agentArea"  name="agentArea"  value="${params.agent_area}"/><label class="must">*</label></li>
			<li><label>超代级别：</label><select name="agentAreaType" id="agentAreaType" style="width:157px;padding: 3px;" onchange="changefrVal(this)">
				<option value='-1'>请选择</option>
				<option value="1" <c:if test="${params.agent_area_type == '1'}">selected="selected"</c:if>>市级超代</option>
				<option value="2" <c:if test="${params.agent_area_type == '2'}">selected="selected"</c:if>>省级超代</option>
			...
			<li><label>超代地址：</label><input type="text" id="agentAddress"  name="agentAddress"  value="${params.agent_address}"/></li>
			<div class="clear"></div>
			<li><label>销售名称：</label><input type="text" id="saleName"  name="saleName"  value="${params.sale_name}"/></li>
			<li><label>超代接入费：</label><input type="text" id="agentPay"  name="agentPay"  value="${params.agent_pay}"/></li>
			<div class="clear"></div>
			...
			<li><label>最小清算金额：</label><input type="text" id="mixAmout"  name="mixAmout"  value="${params.mix_amout}"/></li>
		 	<li  style=" display:none;"><label>上级超代：</label><c:if test="${isParent=='0'}">
			...
			<div class="clear"></div>
			<li><label style="width: 106px;">超代状态：</label>
     ...
     <ul  id="smfr_add_input">
      ...
      <c:set var="smArrayAB" value="${params.smrsharing_rule_chaifen}"></c:set>
       <li style="width: 700px;" ><label>超级刷分润规则：</label> 
      ...
      <!--分润 end    -->
       ...
       <ul  id="ylst_add_input" style="display:none;">
      <!--分润 start-->
      ...
      <div class="title" style="height:20px;"><span style="float:left;width:200px;" >超代行业计算成本</span>
        ...
        <ul  id="feeByYeWu_add_input">
            ...
            <li><label >民生B类 :</label>    <input type="text" name="live_b_type"  value="${params.live_b_type}"  /><label class="must">*</label></li>
            <div class="clear"></div>
            <li><label>批发对公:</label>  <input type="text" name="wholesale_pub_type"  value="${params.wholesale_pub_type}"  /><label class="must">*</label></li>
            <li><label >批发对私封顶类:</label>  <input type="text" name="wholesale_pri_cap_type"  value="${params.wholesale_pri_cap_type}"  /><label class="must">*</label></li>
            <div class="clear"></div>
            ...
            <li><label style="width: 120px">房地产及汽车销售类 :</label> <input type="text"  style="width: 160px;" name="estate_car_type"  value="${params.estate_car_type}"  /><label class="must">*</label></li>
            <div class="clear"></div>
            <li><label>一般A类:</label>  <input type="text" name="general_type"  value="${params.general_type}"  /><label class="must">*</label></li>
            <li><label >一般B类:</label>  <input type="text" name="general_b_type"  value="${params.general_b_type}"  /><label class="must">*</label></li>
            <div class="clear"></div>
            <li><label >餐饮类:</label>  <input type="text" name="catering_type"  value="${params.catering_type}"  /><label class="must">*</label></li>
            <li><label >超级刷:</label>  <input type="text" id="smbox_type" name="smbox_type"  value="${params.smbox_type}"  /><label class="must">*</label></li>
            <!--代理商行业计算成本 end    -->
         </ul>
