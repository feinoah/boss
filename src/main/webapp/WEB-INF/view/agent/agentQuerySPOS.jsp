...
.autocomplete{list-style-type:none; margin:0px; padding:0px; border:#008080 1px solid }
	...
	$(function(){
		...
		{
			$.dialog({title:'超级刷代理商详情',width: 800,height:500,resize: false,lock: true,max:false,content: 'url:agentDetailSPOS?id='+id+'&layout=no'});
		}
		function appendTermianl(id)
		{
			$.dialog({title:'超级刷代理商增机',width: 320,height:180,resize: false,lock: true,max:false,content: 'url:appendTerminalSPOS?agentNo='+id+'&layout=no',close: function(){
							 	$("#submit").click();
					      }});
		}
		...
		function agentFreeze(agentNo){
			...
			$.dialog.confirm('确定要冻结该超级刷代理商吗？', function(){
...
<body>
  <div id="content">
    <div id="nav"><img class="left" src="${ctx}/images/home.gif"/>当前位置：超代管理>超代查询</div>
   
   <form:form id="agentQuery" action="${ctx}/agent/agentQuerySPOS" method="post">
   <div style="position:absolute;margin-left:62px;margin-top:48px;">
    ...
    <div id="search">
    	<div id="title">超代查询</div>
	      <ul>
	      	<li><span >超代：</span><u:select value="${params['agentNo']}"  stype="agent" sname="agentNo"  onlyThowParentAgent="true" /></li>
			<li><span >超代编号：</span><input type="text" style="width:130px" class="input"  value="${params['agentNoText']}" name="agentNoText" onblur="setAgentNo(this);" /></li>
			<li><span  >超代名称：</span><input type="text" style="width:130px" class="input"  value="${params['agentName']}" name="agentName" id="agentName" /></li>
        ...
        <tr><th width="30">序号</th>
          ...
          <c:forEach items="${list.content}" var="item" varStatus="status">
	        <tr  class="${status.count % 2 == 0 ? 'a1' : ''}">
	          ...
	          <td class="center">
	         <shiro:hasPermission name="AGENT_QUERY_DETAIL_SPOS"> <a href="javascript:showDetail(${item.id});">详情</a> |
	         </shiro:hasPermission>
	         <shiro:hasPermission name="AGENT_QUERY_MODIFY_SPOS"> <a href="${ctx }/agent/agentModloadSPOS?id=${item.id}">修改</a>
	          | </shiro:hasPermission>
	          <shiro:hasPermission name="AGENT_QUERY_ADD_SPOS"><a href="javascript:appendTermianlSPOS(${item.agent_no})">增机</a>|</shiro:hasPermission> 
	          <c:if test="${item.isDel=='true'}">
		         <shiro:hasPermission name="AGENT_QUERY_DELETE_SPOS"> <a href="javascript:agentDel(${item.agent_no})">删除</a></shiro:hasPermission>
	          </c:if>
	           <c:if test="${item.isDel=='false'}">
	           	 <shiro:hasPermission name="AGENT_QUERY_FREEZE_SPOS"><a href="javascript:agentFreeze(${item.agent_no})">冻结</a></shiro:hasPermission>
	           </c:if>
	          </td>
	        </tr>
          </c:forEach>
