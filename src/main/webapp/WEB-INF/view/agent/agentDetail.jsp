...
<div class="item liHeight">
	...
	<ul>
	<li style="width:650px;" id="agent_name"><span>代理商名称：</span>${params['agent_name']}</li>
		  <li style="width:650px;" id="pagentname"><span>上级代理商：</span>${params['pagentname']}</li>
		  <li style="width:250px;" id="agent_no"><span>代理商编号：</span>${params['agent_no']}</li>
...
<%--		  <li style="width:200px;" id="parent_id"><span>上级代理商：</span>${params['pagentname']}</li>--%>
		  <li style="width:250px;" id="agent_link_name"><span>联系人：</span>${params['agent_link_name']}</li>
 		  <li style="width:250px;" id="agent_link_tel"><span>联系电话：</span>
			${fn:substring(params['agent_link_tel'],0,3)}****${fn:substring(params['agent_link_tel'],7,11)}
 		  </li>
		  <li style="width:250px;" id="agent_link_mail"><span>联系邮箱：</span>
		  ${fn:substring(params['agent_link_mail'],0,2)}***${fn:substring(params['agent_link_mail'],5,30)}</li>
		  <li style="width:250px;" id="default_password"><span>原始密码：</span>${params['default_password']}</li>
		  <li style="width:250px;" id="agent_area"><span>代理商区域：</span>${params['agent_area']}</li>
		  <li style="width:500px;" id="agent_address"><span>代理商地址：</span>${params['agent_address']}</li>
		  <li style="width:250px;" id="sale_name"><span>销售人员：</span>${params['sale_name']}</li>
		  <li style="width:250px;" id="agent_pay"><span>代理接入费：</span><fmt:formatNumber type="currency" pattern="#,##0.00#" value="${params['agent_pay']}" />元</li>
		  <li style="width:250px;" id="mix_amout"><span style="width: 100">最小清算金额：</span><fmt:formatNumber type="currency" pattern="#,##0.00#" value="${params['mix_amout']}" />元</li>
		  <li style="width:250px;" id="agent_rate"><span>协议费率：</span>${params['agent_rate']}</li>
		  ...
		  <li style="width:250px;" id="sale_name"><span>代理商状态：</span>
		        <c:choose>
				  <c:when test="${params['agent_status'] eq '1'}">正常</c:when>
				  <c:when test="${params['agent_status'] eq '2'}">冻结</c:when> 
			  </c:choose>
		  </li>
		  <li style="width:250px;" id="agent_area_type"><span>代理商级别：</span>
		        <c:choose>
				  <c:when test="${params['agent_area_type'] eq '1'}">市级代理</c:when>
				  <c:when test="${params['agent_area_type'] eq '2'}">省级代理</c:when> 
			  </c:choose>
		  </li>
		...
		<li style="width:750px;"><span style="width: 100">移小宝分润规则：</span>
		...
		<li style="width:750px;"><span style="width: 110">移联商通分润规则：</span>
     ...
     <!--代理商行业计算成本 start-->
            ...
            <li  style="width: 750px;"><span style="width: 100">&nbsp;</span>
               ...
               <label>移小宝    </label>  ${params.smbox_type}&nbsp;&nbsp;
               <label>移联商通II    </label>  ${params.ylst2_type}&nbsp;&nbsp;
            </li>  
     <!--代理商行业计算成本 end    -->
