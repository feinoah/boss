...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#merchant_info").on("input","input[name=merchantName]",function(){
			...
			$.post(root+"/merchantApi/searchMerchant","kw="+this.value,function(data){
				if(data.length>0){
					for(var i in data){
		 ...
		 $("#addButton").on("click",function(){
			...
			if(!selected){
				...
				if(pass){
					...
					$(this).off("click");
					$.post(root+"/merchantApi/merchantApiAdd",data,function(data){
						if(data.success){
...
<body>
	<div id="content">
		<div id="nav">
			<img class="left" src="${ctx}/images/home.gif" />当前位置：系统管理&gt;商户接口新增
		</div>
