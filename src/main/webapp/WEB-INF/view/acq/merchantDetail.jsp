...
<div class="item liHeight">
	...
	<ul>
		  ...
		  <li style="width:300px;" id="acq_enname"><span style="width:120px;">收单机构：</span>
		  		<u:acqOrgName svalue="${params.acq_enname}" />
				...
				<c:if test="${params['acq_enname'] eq 'xlink'}">讯联</c:if>
				<c:if test="${params['acq_enname'] eq 'zypay'}">中意支付</c:if>
				<c:if test="${params['acq_enname'] eq 'ubs'}">RYX</c:if>
				<c:if test="${params['acq_enname'] eq 'hypay'}">翰亿</c:if>
