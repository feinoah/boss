...
<script type="text/javascript">
	...
	function del(merchant_id) {
		$.dialog.confirm('确定要删除商户接口吗？', function() {
			...
			$.get(root + "/merchantSdkApi/delMerchantSdkApi", data, function(data) {
				if (data.success) {
	...
	function changeForbidden(status, merchant_id) {
		...
		$.get(root + "/merchantSdkApi/changeForbindden", data, function(data) {
			if (data.success) {
				showInfo("操作成功", function() {
					$("#merQuery").submit();
				}, "success.gif");
...
<body>
	<div id="content">
		<div id="nav">
			<img class="left" src="${ctx}/images/home.gif" />当前位置：系统管理&gt;商户sdk接口查询
		</div>
		<form id="merQuery" action="${ctx}/merchantSdkApi/merchantSdkApiQuery" method="post">
			<div id="search">
			...
			<div class="search_btn">
				<input class="button blue medium" type="button" onclick="location.href='${ctx}/merchantSdkApi/merchantSdkApiAddPage'" value="添加" />
					...
					<tr>
						...
						<c:forEach items="${list.content}" var="item" varStatus="status">
							<tr class="${status.count % 2 == 0 ? 'a1' : ''}">
								...
								<td class="center"><a href="${ctx}/merchantSdkApi/merchantSdkApiDetail?id=${item.id}">详情</a> <c:choose>
									...
									</c:choose> <a href="javascript:del(${item.id});">删除</a> <a href="${ctx}/merchantSdkApi/goEditPage?id=${item.id}">修改</a></td>
							</tr>
						</c:forEach>
			</table>
		</div>
