...
<body>
  <div id="content">
    <div id="nav"><img class="left" src="${ctx}/images/home.gif"/>当前位置：商户管理>商户提额审核</div>
    
    <form:form id="merCheck" action="${ctx}/mer/raiseCheckQuery" method="post">
    	...
    	<div id="title">商户提额审核</div>
	      <ul>
	         ...
	         <li><span style="width: 90px;">商户名称/编号：</span><input type="text"  value="${params['merchant']}" name="merchant" /></li>
	      	<li><span>创建时间：</span>
			 	<input  type="text" style="width:102px" name="create_time_begin" class="input" readonly="readonly"  value="${params['create_time_begin']}" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})">
			 	~
			 	<input  type="text" style="width:102px" name="create_time_end" class="input" readonly="readonly"  value="${params['create_time_end']}" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})">
			 </li>
			 <li><span>审核状态：</span> 
	         	<select style="padding:2px;width:140px" name="check_status">
	         		<option value="-1" <c:out value="${params['check_status'] eq '-1'?'selected':'' }"/>>全部</option>
	         		<option value="3" <c:out value="${params['check_status'] eq '3'?'selected':'' }"/>>复审中</option>
	         		<option value="4" <c:out value="${params['check_status'] eq '4'?'selected':'' }"/>>审核失败</option>
	         		<option value="5" <c:out value="${params['check_status'] eq '5'?'selected':'' }"/>>审核通过</option>
	         	</select>
			 </li>
	      </ul>
	       <div class="clear"></div>
        ...
        <tr>
 		...
 		<c:forEach items="${list.content}" var="item" varStatus="status">
	        ...
	        <tr  class="${status.count % 2 == 0 ? 'a1' : ''}">
	          ...
	          <td align="center">${item.checker}</td>
	          <td  align="center">
	          	<c:choose>
				  <c:when test="${item.check_status eq '1'}">已申请</c:when>
				  <c:when test="${item.check_status eq '2'}">初审失败</c:when>
				  <c:when test="${item.check_status eq '3'}">复审中</c:when>
				  <c:when test="${item.check_status eq '4'}">复审失败</c:when>
				  <c:when test="${item.check_status eq '5'}">审核通过</c:when>    
				  <c:otherwise>${item.check_status }</c:otherwise>
			  	</c:choose>
	          </td>
	          <td><fmt:formatDate value="${item.create_time}" pattern="yyyy-MM-dd HH:mm:ss" type="both"/></td>
	          <td class="center">
	          	<c:if test="${item.check_status eq '3'}">
	          		<a href="javascript:location.href= '${ctx}/mer/raiseCheckDetail?id=${item.id}';">审核</a></td>
	          	</c:if>
	          	<c:if test="${item.check_status ne '3'}">
	          		<a href="javascript:location.href= '${ctx}/mer/raiseCheckInfo?id=${item.id}';">查看</a></td>
	          	</c:if>	         
	        </tr>
         </c:forEach>
      </table>
