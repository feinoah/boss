...
<script type="text/javascript">
	...
	function exportExcel2() {
		...
		exportXls('${ctx}/mer/export', ${list.totalPages}, false);
	}
	//创建统计信息html结构文件
	...
	function showSignReceipt2(id) {
		signRecepit_dialog = $.dialog({
			...
			lock : true,
			max : false,
			content : 'url:receiptReviewLoad2/' + id + '?layout=no'
		});
	}
	//冻结交易
...
<body>
	<div id="content">
		<div id="nav">
			<img class="left" src="${ctx}/images/home.gif" />当前位置：商户管理>交易查询（POS）
		</div>
		<form:form id="trans" action="${ctx}/mer/trans" method="post">
				...
				<ul>
					...
					<li style="width: 256px;">
						...
						<%--<select style="padding: 2px; width: 178px" id="transSource" name="transSource">
					        ...
					        <option value="M188" <c:out value="${params['transSource'] eq 'M188'?'selected':'' }"/>>超级刷III</option>
						...
						${item.trans_amount}
								...
								<td class="center"><shiro:hasPermission name="COMMERCIAL_TRANS_DETAIL">
									...
									<shiro:hasPermission name="COMMERCIAL_SIGN_RECEIPT">
										...
										<c:if test="${item.trans_source eq 'DOT'&&item.trans_type ne 'BALANCE_QUERY'&&item.trans_status ne 'FAILED'}">
											<a href="javascript:showSignReceipt('${item.id}');">小票</a>
										</c:if>
										<c:if test="${item.trans_source eq 'SPOS'&&item.trans_type ne 'BALANCE_QUERY'&&item.trans_status ne 'FAILED'}">
											<a href="javascript:showSignReceipt('${item.id}');">小票</a>
										</c:if>
										<c:if test="${item.trans_source eq 'SPOS_2'&&item.trans_type ne 'BALANCE_QUERY'&&item.trans_status ne 'FAILED'}">
											<a href="javascript:showSignReceipt('${item.id}');">小票</a>
										</c:if>
										<c:if test="${item.trans_source eq 'NEW_LAND_ME30'&&item.trans_type ne 'BALANCE_QUERY'&&item.trans_status ne 'FAILED'}">
						                     <a href="javascript:showSignReceipt('${item.id}');">小票</a>
					                     </c:if>
										<c:if test="${item.trans_source eq 'NEW_LAND_ME31'&&item.trans_type ne 'BALANCE_QUERY'&&item.trans_status ne 'FAILED'}">
											<a href="javascript:showSignReceipt('${item.id}');">小票</a>
										</c:if>
										<c:if test="${item.trans_source eq 'TY'&&item.trans_type ne 'BALANCE_QUERY'&&item.trans_status ne 'FAILED'}">
											<a href="javascript:showSignReceipt('${item.id}');">小票</a>
										</c:if>
										<c:if test="${item.sign_img != null &&item.sign_img ne ''&& item.trans_source eq 'POS' && item.trans_status ne 'FAILED'}">
						                     <a href="javascript:showSignReceipt('${item.id}');">小票</a>
					                     </c:if>
									</shiro:hasPermission>
										<c:if test="${item.trans_status eq 'SUCCESS' }">
												<shiro:hasPermission name="COMMERCIAL_SIGN_RECEIPT2">
													<a href="javascript:showSignReceipt2('${item.id}');">小票2</a>
												</shiro:hasPermission>
									</c:if>
									...
									<td>
										<c:choose>
											<c:when test="${item.review_status eq '1'}">审核通过</c:when>
											<c:when test="${item.review_status eq '2'}">未审核</c:when>
											<c:when test="${item.review_status eq '3'}"><span class="font_red">审核未通过</span></c:when>
											<c:when test="${item.review_status eq '4'}">已重签</c:when>
											<c:otherwise>${item.review_status}</c:otherwise>
										</c:choose>
									</td>
							</tr>
						</c:forEach>
