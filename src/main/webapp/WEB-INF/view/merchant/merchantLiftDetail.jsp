...
<style type="text/css">
#liftDetailSubmit ul li label.must {
...
#liftDetailSubmit ul li.column2 {
	width: 500px;
}

#liftDetailSubmit ul li.column3 {
	width: 750px;
}

#liftDetailSubmit ul li label {
	display: -moz-inline-box;
	display: inline-block;
	width: 90px;
}

#liftDetailSubmit ul li .area {
	width: 75px;
	height: 24px;
	vertical-align: top;
}
...
<script type="text/javascript">
	...
	$(function(){
		$("#submitFail").on('click', function() {
			var failSubmit = true;
			if (!$.trim($("#examination_opinions").val())) {
				var dialog = $.dialog({title : '错误',lock : true,content : '审核意见不能为空',icon : 'error.gif',
					ok : function() {$("#examination_opinions").focus();}});
...
<body>
	<div id="content">
		<div id="nav">
			<img class="left" src="${ctx}/images/home.gif" />当前位置：商户管理 &gt;额度提升审核
		</div>
		<form id="liftDetailSubmit" name="liftDetailSubmit"   action="${ctx}/mer/liftDetailSubmit" method="post">
			...
			<div class="search_btn">
				<shiro:hasPermission name="COMMERCIAL_LIFT_CHECK_OK">
