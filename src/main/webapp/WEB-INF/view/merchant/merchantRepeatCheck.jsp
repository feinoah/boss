...
<script type="text/javascript" src="${ ctx}/scripts/utils.js"></script>
	...
	function checkRepeatDetail(id){
		//$.dialog({lock: true,drag: false,resize: false,max: false,content: 'url:checkRepeatDetail?id='+id+'&layout=no',close: function(){
		$.dialog({height:600,width: 900,lock: true,drag: false,resize: false,max: false,content: 'url:checkRepeatDetail?id='+id+'&layout=no',close: function(){
      }});
	}
...
<body>
  <div id="content">
    <div id="nav"><img class="left" src="${ctx}/images/home.gif"/>当前位置：商户管理>商户复审</div>
    
    <form:form id="merCheck" action="${ctx}/mer/checkRepeatQuery" method="post">
        ...
        <tr>
 		...
 		<c:forEach items="${list.content}" var="item" varStatus="status">
	        ...
	        <tr  class="${status.count % 2 == 0 ? 'a1' : ''}">
	          ...
	          <td class="center"> <shiro:hasPermission name="COMMERCIAL_CHECK_CHECK"><a href="javascript:checkRepeatDetail(${item.id});">复审</a></shiro:hasPermission></td>	         
	        </tr>
         </c:forEach>
